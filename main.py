...
# Import the RGBLed class
from RGBLed import RGBLed

# Define the RGLed pins, as required for the board specifics
Red_pin = 14
Green_pin = 13
Blue_pin = 4

# Predefine some handy simple colours - others are available! See lib.RGBLed.py for more 
red = [255,0,0]
green = [0,255,0]
blue = [0,0,255]
...
async def wait_press(e, lcd):
    """ wait_press: Rotates around IPAddr, SSID/Hostname, MOTD """
    state = 0
    while True:
        await e.wait()
        e.clear()
        ...
        if state == 0:
            ...
            wifiInfo = f"Mode:{WiFiConnection.getMode()} SSID:{WiFiConnection.ssid} Host:{WiFiConnection.hostname}"
...
async def main():
    ...
    ************************************************
    ...
    rgb = RGBLed(Red_pin,Green_pin,Blue_pin) # Configure the RG LED pins
