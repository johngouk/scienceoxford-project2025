...
"""
    Modified 2024-10-21 John Gouk
    Now only sets Events or calls callbacks for actual events
    i.e. Press only indicated for Presses, not for the first of a Double or the start of a Long,
    and so on.
    Really requires rewriting with FSMs for each operation type, which might clarify all those
    horrible flags!
...
from . import launch, Delay_ms # launch comes from __init__.py, a bit too clever by half
...
class Pushbutton:
    debounce_ms = 50
    long_press_ms = 700 # JG Was 1000
    double_click_ms = 300 # JG Was 400
    ...
    def _check(self, state):
        ...
        if state:  # Button pressed: launch pressed func
            ...
            # JG a long start... so remember we might have to register a Press
            if self._tf:
            ...
            # JG A double click is pending of _dblpend == True! Make it easy
            ...
            if self._df:
                if self._dd():  # Second click: timer running
                    ...
                    self._press_pending = False # JG We now see that this wasn't a Press, but a Double
            ...
            if self._ld:
                ...
                # JG and we won't see that here!
                self._long_pending = False # JG Nah, no need to wait, button released
                # JG If Press enabled and waiting to confirm Press and we're not waiting for Double...
                # JG We have a Press!
    ...
    def _ddto(self):  # Doubleclick timeout: no doubleclick occurred
        ...
        if self._tf and self._press_pending and not (self._ld and self._long_pending): # JG
            self._press_pending = False # JG
